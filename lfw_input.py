...
def read_image_from_disk(filename_to_label_tuple):
    ...
    # FIX: universal decoder instead of decode_jpeg
...
def filter_dataset(dataset, min_images_per_label=10):
    ...
    for cls in dataset:
        ...
        if len(cls.image_paths) < min_images_per_label:

            logger.info('Skipping class: {}'.format(cls.name))
